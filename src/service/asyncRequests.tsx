...
export const fetchNewActivity = (type: string) => {
  ...
  return (dispatch: Dispatch<MainActions| ServiceActions>  ) => {
    fetch(`https://www.boredapi.com/api/activity?${data}`, {
      ...
      .then((res) => res.json())
       .then((data) => dispatch({type: ActionTypes.GET_ACTIVITY, payload: data}))
      .then(() =>
        setTimeout  ( () => {  
...
export const sendActivityWithSuggestion = (suggestion: object) => {
  return (dispatch: Dispatch<ServiceActions> ) => {
    fetch(`https://www.boredapi.com/api/suggestion`, {
      ...
      .then((res) => res.json())
      .then((res) => dispatch({ type: ActionTypes.SUGGEST_RESPONSE, payload: res }))
